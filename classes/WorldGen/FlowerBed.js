import * as THREE from '../../modules/three.module.js';

class FlowerBed{
    constructor(pos){
        this.model = new THREE.Object3D();
        this.FlowerBed(pos);
    }
    FlowerBed(pos){
        
        var flower = this.createFlower()
        
        var obj = new THREE.Group();  //adds truck and leaves to a single group
        // tree.add(treeLeave);
        
        obj.add(flower);
        obj.position.x = pos.x;
        obj.position.y = pos.y;//same pos
        obj.position.z = pos.z;
        this.model.add(obj);
    }

    createFlower(){
        var leaves = this.createLeaves();
        var stem = this.createStem();
        var seeds = this.createSeed();
        var flower = new THREE.Group()
        flower.add(leaves);
        flower.add(stem);
        flower.add(seeds);
        return flower;
    }

    createLeaves(){
        var sphereMat = new THREE.MeshPhongMaterial();
        sphereMat.side = THREE.DoubleSide;
        // sphereMat.wireframe = true;
        sphereMat.color = new THREE.Color(0xfdf35e);
        var spherGeo = new THREE.SphereGeometry(5, 6, 2, 0, Math.PI*2, 0, 1);
        var sphere = new THREE.Mesh(spherGeo, sphereMat);
        sphere.rotateZ(Math.PI/2);
        sphere.position.x +=5;
        return sphere;
    }

    createStem(){
        var stemMat = new THREE.MeshBasicMaterial();
        stemMat.color = new THREE.Color(0x008000);
        var stemGeo = new THREE.CylinderGeometry(0, 0.5, 10, 3);
        var stem = new THREE.Mesh(stemGeo, stemMat);
        stem.position.y -=5;
        
        return stem;
    }

    createSeed(){



        var seedMat = new THREE.MeshBasicMaterial();
        seedMat.color = new THREE.Color(0x343434);
        var seedGeo = new THREE.SphereGeometry(1.5, 3, 3);
        var seed = new THREE.Mesh(seedGeo, seedMat);
        seed.position.x += 1.5;

        return seed;
    }


}

export {FlowerBed};